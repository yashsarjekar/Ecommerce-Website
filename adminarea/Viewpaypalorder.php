...
while($rowquery=mysqli_fetch_array($runquery)){
	$Payment_id=$rowquery['payment_id'];
	$customer_id=$rowquery['transection_no'];
	$invoice_no=$rowquery['amount'];
	$product_id=$rowquery['customer_id'];
	$qty=$rowquery['product_id'];
	$order_status=$rowquery['qty'];
		$order_status1=$rowquery['status'];
			$order_status2=$rowquery['currency'];
	?>
<tr>
...
<td><center><?php echo $order_status1;?></center></td>
<td><center><?php echo $order_status2;?></center></td>
</tr>	
<?php }?>
</table>
<?php }else{
